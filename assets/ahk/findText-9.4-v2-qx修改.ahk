...
{  ;// Class Begin
    ...
    /**
     * 屏幕找字函数 v9.4 (2023-12-18)
     * @param {String} OutputX 1.保存返回的X坐标的变量名称; 2.等于 'wait' 或 'wait1' 意味着等待图像出现; 2.等于 'wait0' 意味着等待图像消失,此时返回1
     * @param {String} OutputY 1.保存返回的Y坐标的变量名称; 2.设置等待时间的秒数，如果小于0则无限等待
     * @param {Integer} x1 查找范围的左上角X坐标
     * @param {Integer} y1 查找范围的左上角Y坐标
     * @param {Integer} x2 查找范围的右下角X坐标
     * @param {Integer} y2 查找范围的右下角Y坐标
     * @param {Integer} err1 文字的黑点容错百分率（0.1=10%）
     * @param {Integer} err0 背景的白点容错百分率（0.1=10%）
     * @param {String} text 由工具生成的查找图像的数据，可以一次查找多个，用“|”分隔
     * @param {Integer} ScreenShot 是否截屏，为0则使用上一次的截屏数据
     * @param {Integer} FindAll 是否搜索所有位置，为0则找到一个位置就返回
     * @param {Integer} JoinText 如果想组合查找，可以为1，或者是要查找单词的数组
     * @param {Integer} offsetX 组合图像的每个字和前一个字的最大横向间隔
     * @param {Integer} offsetY 组合图像的每个字和前一个字的最大高低间隔
     * @param {Integer} dir 查找的方向，有上、下、左、右、中心9种
     * @param {Integer} zoomW 图像宽度的缩放百分率（1.0=100%）
     * @param {Integer} zoomH 图像高度的缩放百分率（1.0=100%）
     * @returns {Any|Integer|Array} 没找到返回0,否则返回一个二维数组,第一级是每个结果对象，第二级是结果对象的具体信息对象:{ 1:左上角X, 2:左上角Y, 3:图像宽度W, 4:图像高度H, x:中心点X, y:中心点Y, id:图像识别文本 }
     */
                ...
                For i, j in info
                    Loop this.PicFind(ini, j, dir, sx, sy, sw, sh, allpos_ptr)
                    {
                        pos := NumGet(allpos, 4 * (A_Index - 1), "uint")
                            ...
                            , arr.Push({ 1: x, 2: y, 3: w, 4: h, x: x + w // 2, y: y + h // 2, id: comment, width:w, height:h,posX:x,poxY:y,index:A_Index })
                        if (!FindAll)
                            Break 3
                    }
            }
     ...
     * 当找到任意目标时，返回该目标的相关信息
     * @param args 任意多的字符串
     * @returns {Integer} 返回找到的第一个目标, 否则返回 false
     */
    any(args*) {
        this.ScreenShot()
        for i, v in args {
     ...
     * 当所有目标都被找到时，返回 true
     * @param args 任意多的字符串
     * @returns {Integer} 全部找到时返回true, 否则返回 false
     */
    all(args*) {
        this.ScreenShot()
        for i, v in args {
 ...
 /**
    ...
    /**
     * 从屏幕中截取指定区域的像素数据，并返回代表屏幕截图数据的对象。
     * @param {Integer} x 截取屏幕像素的起始横坐标
     * @param {Integer} y 截取屏幕像素的起始纵坐标
     * @param {Integer} w 截取屏幕像素的宽度
     * @param {Integer} h 截取屏幕像素的高度
     * @param {Integer} ScreenShot 用于指定是否进行屏幕截图，默认值为 1
     * @param {Integer} zx 传入/传出参数，表示屏幕的横坐标
     * @param {Integer} zy 传入/传出参数，表示屏幕的纵坐标
     * @param {Integer} zw 传入/传出参数，表示屏幕的宽度
     * @param {Integer} zh 传入/传出参数，表示屏幕的高度
     */
        ...
        , ScreenShot := 1, &zx := 0, &zy := 0, &zw := 0, &zh := 0)
    {
            ...
            && GetBitsFromScreen2(bits, x - zx, y - zy, w, h)
        {
            ...
            ; copied to the screenshot cache using this.CopyBits()
             ; 每次从 DXGI 获取到的小范围数据都必须使用此.CopyBits() 复制到截图缓存中
            zx := bits.zx, zy := bits.zy, zw := bits.zw, zh := bits.zh
            Critical(cri)
            return bits
        }
     ...
     * @param {Integer} w 
     * @param {Integer} h 
     * @param {Integer} weight 边框的粗细
     */
    MouseTip(x := "", y := "", w := 10, h := 10, weight := 3)
    {
        if (x = "")
        {
        ...
        Loop 4
        {
            this.RangeTip(x - w, y - h, 2 * w + 1, 2 * h + 1, (A_Index & 1 ? "Red" : "Blue"), weight)
            Sleep 500
        }
        this.RangeTip()
    }
     ...
     * @param {Integer} weight  边框的粗细
     */
    RangeTip(x := "", y := "", w := "", h := "", color := "Red", weight := 3)
    {
        static Range := Map()
        if (x = "")
        {
        ...
        x := (IsNumBer(x) ? x : 0), y := (IsNumBer(y) ? y : 0)
            , w := (IsNumBer(w) ? w : 0), h := (IsNumBer(h) ? h : 0), weight := (IsNumBer(weight) ? weight : 0)
        Loop 4
        {
            i := A_Index
                , x1 := (i = 2 ? x + w : x - weight)
                , y1 := (i = 3 ? y + h : y - weight)
                , w1 := (i = 1 || i = 3 ? w + 2 * weight : weight)
                , h1 := (i = 2 || i = 4 ? h + 2 * weight : weight)
    ...
    ; 贴图
    ShowScreenShotPin(x1 := 0, y1 := 0, x2 := 0, y2 := 0, ScreenShot := 1)
    {
        static hPic, oldx, oldy, oldw, oldh, FindText_Screen := ""
        x1 := this.Floor(x1), y1 := this.Floor(y1), x2 := this.Floor(x2), y2 := this.Floor(y2)
        if (x1 = 0 && y1 = 0 && x2 = 0 && y2 = 0)
        ...
        {
            FindText_Screen := _Gui := Gui()  ; WS_EX_NOACTIVATE:=0x08000000
            _Gui.Opt "+AlwaysOnTop -MaximizeBox -MinimizeBox -DPIScale"
            _Gui.Name := "FindText_Screen"
            _Gui.MarginX := 0, _Gui.MarginY := 0
            id := _Gui.Add("Pic", "w" w " h" h), hPic := id.Hwnd
            _Gui.Title := "Show Pic"

            _Gui.OnEvent("Close", (*) => (this.ShowScreenShotPin()))
            _Gui.OnEvent("Escape", (*) => (this.ShowScreenShotPin()))

            _Gui.Show "NA x" x " y" y " w" w " h" h
            WinSetTransparent 230, "Show Pic"
    ...
    Gui(cmd, arg1 := "", args*)
    {
        ...
        Switch cmd, 1
        {
            ...
            Case "MakeMainWindow":
                ...
                Gui_G()
                _Gui.Add "Button", "x+0 wp vCaptureTimeout", Lang["CaptureTimeout"]
                Gui_G()
                ...
                Gui_G()
                _Gui.Add "Button", "x+0 wp vCopyText", Lang["CopyText"]
                Gui_G()
            ...
            Case "Copy","CopyText":
                ...
                if (s = "")
                {
                    ...
                    if (r != 1){
                        ...
                        ; 将图像 ID 附加到命名中
                        if RegExMatch(s,"\|<(.*)>",&reg_res){
                            s := StrReplace(s,"X,","X" reg_res[1] ",")
                            s := StrReplace(s,"Y,","Y" reg_res[1] ",")
                            s := RegExReplace(s,"(?<!Find)Text","Text" reg_res[1])
                            s := StrReplace(s,"ok:=","ok" reg_res[1] ":=")
                        }
                        ; 仅复制 Text 
                        if(cmd=="CopyText"){
                            s := RegExReplace(s, "i)\nif[\s\S]*")
            ...
            Case "OK", "SplitAdd", "AllAdd":
                ...
                ; if Comment {
                ;     s := StrReplace(s, "Text:=", "Text" . Comment . ":=")
                ; }
                ...
                ; if Comment {
                ;     s := StrReplace(s, "Text)", "Text" . Comment . ")")
                ;     s := StrReplace(s, "x,", "x" . Comment . ",")
                ;     s := StrReplace(s, "y,", "y" . Comment . ",")
                ; }
                Event := cmd, Result := s
                _Gui.Hide
                return
