...
Filerename(){
    ...
    fNameReName(*) {
        ...
        newNameFull := fName.parentPath . "\" . newName
        ; netPath(fName.sel)
        ; MsgBox fName.sel . "`n" . newNameFull
        ; MsgBox newNameFull
        try{
    ...
    netPath_check(path){
        ...
        ; MsgBox OutDrive . "`n" . DriveType

        ; MsgBox DriveGetSerial(OutDrive)
        ; 检查驱动器类型
        if (!driveType) {
            ; MsgBox (OutDrive . " 是一个映射的网络驱动器！")
            if (SubStr(OutDrive, 1, 2) != "\\") {
                ; MsgBox("字符串的最前面不是 \\")
                MsgBox("启动失败:`n由于是该文件在映射的网络驱动器中, 目前脚本只支持UNC路径,类似\\Workstation01, 而非 " . OutDrive . " 这种。可以从网络面板中重新找到该文件进行修改。")
                Exit
            }
        }
    }
